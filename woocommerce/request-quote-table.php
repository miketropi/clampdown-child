						...
						<?php
							...
							<tr class="<?php echo esc_attr( apply_filters( 'yith_ywraq_item_class', 'cart_item', $raq_content, $key ) ); ?>" <?php echo esc_attr( apply_filters( 'yith_ywraq_item_attributes', '', $raq_content, $key ) ); ?>>
								...
								<td class="product-name"
									...
									<?php
									echo apply_filters( 'yith_ywraq_item_remove_link', sprintf( '<a href="#"  data-remove-item="%s" data-wp_nonce="%s"  data-product_id="%d" class="yith-ywraq-item-remove remove __custom-remove-item" title="%s">Remove Item</a>', esc_attr( $key ), esc_attr( wp_create_nonce( 'remove-request-quote-' . $product_id ) ), esc_attr( $product_id ), esc_attr__( 'Remove this item', 'yith-woocommerce-request-a-quote' ) ), $key ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
									?>

									<?php 
									/**
									 * Clampdown child custom code 
									 * 
									 * clampdown_child/request_quote/custom_name_item_after hook 
									 * @see clampdown_child_woo_render_request_quote_pricing_data_item - 20
									 */
									do_action('clampdown_child/request_quote/custom_name_item_after', $raq, $key); 
									?>
								</td>
